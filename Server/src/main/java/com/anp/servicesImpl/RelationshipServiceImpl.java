...
import com.anp.repositories.RelationshipRepository;
...
public class RelationshipServiceImpl implements RelationshipService {
    ...
    @Override
    public List<ConnectionCandidatesViewModel> searchUsers(String loggedInUserId, String search) {
        ...
        return userList.stream()
                .map(currentUser -> this.modelMapper.map(currentUser, ConnectionCandidatesViewModel.class))
                .map(user -> mapUser(user, currentUserRelationshipList))
                .collect(Collectors.toList());
    }

    @Override
    public List<ConnectionCandidatesViewModel> findAllFriendCandidates(String loggedInUserId) {
        ...
        List<ConnectionCandidatesViewModel> notFriendsUserList = userList.stream()
                .filter(user -> !usersWithRelationship.contains(user) && !user.getId().equals(loggedInUserId))
                .map(user -> this.modelMapper.map(user, ConnectionCandidatesViewModel.class))
                .collect(Collectors.toList());
    ...
    @Override
    public boolean createRequestForAddingFriend(String loggedInUserId, String connectionCandidateId) throws Exception {
        User loggedInUser = this.userRepository.findById(loggedInUserId)
                .filter(userValidation::isValid)
                .orElseThrow(Exception::new);

        User friendCandidateUser = this.userRepository.findById(connectionCandidateId)
                .filter(userValidation::isValid)
                .orElseThrow(Exception::new);

        Relationship relationshipFromDb = this.relationshipRepository.findRelationshipByUserOneIdAndUserTwoId(loggedInUserId, connectionCandidateId);
    ...
    private ConnectionCandidatesViewModel mapUser(ConnectionCandidatesViewModel user, List<Relationship> relationshipList) {
